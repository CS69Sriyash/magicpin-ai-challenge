...
def print_score_bar(dimension: str, score: int, max_score: int = 10):
    ...
    print(
        f"  {dimension:22} [{color}{'█' * bar_filled}{Colors.DIM}{'░' * bar_empty}{Colors.RESET}] {color}{score:2}/{max_score}{Colors.RESET}"
    )
...
class BotClient:
    ...
    def _request(
        self, method: str, path: str, timeout: int = 30, body_dict: Dict = None
            ...
            result = ScoreResult(
                ...
                decision_quality=min(
                    ...
                    max(
                        ...
                        int(
                            data.get(
                                "decision_quality", data.get("trigger_relevance", 5)
                            )
                        ),
                    ),
                ),
                decision_quality_reason=data.get(
                    "decision_quality_reason", data.get("trigger_relevance_reason", "")
                ),
                engagement_compulsion=min(
                    10, max(0, int(data.get("engagement_compulsion", 5)))
                ),
...
class JudgeSimulator:
    ...
    def _warmup(self) -> bool:
        ...
        if err:
            ...
            print_success(
                f"metadata — Team: {data.get('team_name', '?')}, Model: {data.get('model', '?')}"
            )
    ...
    def _auto_reply(self) -> bool:
        ...
        mid = (
            list(self.dataset.merchants.keys())[0]
            if self.dataset.merchants
            else "m_test"
        )
    ...
    def _intent(self) -> bool:
        ...
        if any(w in body_lower for w in actioning) and not any(
            w in body_lower for w in qualifying
        ):
    ...
    def _hostile(self) -> bool:
        ...
        elif action == "send" and any(
            w in body.lower() for w in ["sorry", "apolog", "won't"]
        ):
    ...
    def _final_summary(self):
        ...
        avg = ScoreResult(
            ...
            engagement_compulsion=sum(s.engagement_compulsion for s in self.all_scores)
            // n,
...
def main():
    ...
    try:
        test_response = llm.complete(
            "Say 'ready' if you can hear me.", "You are a test assistant."
        )
        if test_response:
